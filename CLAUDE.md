...
The `hari-core` binary has seven modes selected by the first positional argument: no args runs the **substrate-decision demo** (`run_substrate_decision_demo` in `main.rs` — a self-referential scripted scenario showing Hari tracking claims about its own Phase 5 substrate decision via `RelationDeclaration` + `AgentVote` events under `TrustModel::RoleWeighted`); `replay <path>` deserializes a `ResearchTrace` (object form) or a bare `Vec<ResearchEvent>` (array form) and emits a `ResearchReplayReport` as pretty JSON; `serve` runs the Phase 6 stdio JSONL streaming protocol; `forecast <emit|resolve|calibration>` operates the Jarvis Track J2 forecast ledger (`crates/hari-core/src/forecast.rs`, contract: `ga:docs/contracts/2026-07-02-hari-forecast-record.contract.md` v0.1 DRAFT, hari-side review: `docs/research/2026-07-02-forecast-record-hari-review.md`); `reliability <grades-dir> [--now <rfc3339Z>]` (Giskard Track G2, `crates/hari-core/src/reliability.rs`) reads GA's `pr-grade-v1` cards (`ga:state/quality/pr-grades/`) and prints a per-agent × task-class reliability report — read-only, cards without the additive `agent` field degrade to `"unknown"`, and the `pooled` entry is the simpler baseline any consumer must beat (A/B doctrine); `operator <surface|ack|status>` (Giskard Track G1 ops face, `crates/hari-core/src/operator_model.rs`) is a seen/acknowledged/stale registry over signals surfaced to the human operator, appended to an injected-timestamp JSONL ledger under `HARI_STATE_DIR/operator-model/`, exposing one consuming decision (`should_notify` = don't re-notify what's already acknowledged) plus `is_stale` — `status` is read-only, surface/ack append; substrate half only (the GA producer/consumer halves are a later slice); `source-reliability <emit|report>` (issue #14, `crates/hari-core/src/source_reliability.rs`, contract: `docs/contracts/source-reliability-summary.contract.md` v0.1 DRAFT, design: `docs/research/2026-07-20-source-reliability-design.md`) generalises G2 from agents to any `ResearchEvent.source`: `emit --trace <path> --session <id>` replays a trace and appends per-source outcome rows (claims/accepted/false_acceptances/escalations) to an append-only JSONL ledger under `HARI_STATE_DIR/source-reliability/`, `report` aggregates the ledger across sessions into per-source precision with the `pooled` A/B baseline plus a read-only **entrenchment ordering** (source reliability = AGM entrenchment, compounding-strategy §5) — surfaced, never auto-applied (no default trust change; full trust-calibration integration awaits owner review). When adding new event types, update both `parse_trace` paths.
...
GitHub Issues on `GuitarAlchemist/hari`, via the `gh` CLI. See `docs/agents/issue-tracker.md`.
