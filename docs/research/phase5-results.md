...
**Reframe needed.** The mechanical exit criterion (divergence + boundedness) was satisfied and Phase 5 is technically complete. The §7b finding on `long_recovery.json` even produced what looked like the first measurable Lie advantage (false_acceptance_count 3 vs RecencyDecay's 4). But the subsequent Subjective Logic baseline (see `docs/research/phase5-fixture-rollup.md` §7, added 2026-04-29) inverted that result:

- **SL beats Lie on the same metric**, on 3 of 6 fixtures, ties on 3, never loses. SL also beats Lie on `contradiction_recovery_cycles` on `long_recovery.json` (4 vs 5).
- On every other comparable metric (`goal_completion_rate`, `consensus_stability`), Lie ties both RecencyDecay and SL.
- ~600 lines of Subjective Logic — no algebra, no Lie groups, no Hamiltonian — beats ~470 LOC of Lie wiring + the entire `hari-cognition` crate on the only metric where Lie appeared to have an edge against the strawman baseline.

Updated reading:

- **The hari-cognition crate does not currently pay for its complexity** as a decision engine. The original Phase 5 hypothesis ("Lie-inspired state evolution improves research decisions") is not supported by the empirical comparison against credible prior art.
- **The Lie machinery remains mathematically real**, but if it has value, that value lies somewhere other than `false_acceptance_count` against an SL competitor. Untested alternative framings: interpretability of the attention trajectory, smooth-state continuity preservation, structure-constant-driven analysis of cognitive-op commutativity. The project owner needs to decide whether any of these are worth instrumenting.
- **Phase 6's default cognitive engine should not be `Lie`.** As of this update, the default `PriorityModel` remains `Flat` — a project-direction call about whether to switch the default to `RecencyDecay` or `SubjectiveLogic` is open. The Lie machinery stays in place as an experimental knob; the `divergence_test_pins_alpha_and_dt` test still asserts the α=2.0/dt=0.5 defaults so any future tuning is explicit.
- **The original "tighten the coupling" remediations from this section's v1** — widen generator basis (a), longer recovery fixture (b), swarm-consensus Hamiltonian (c), strengthen Hamiltonian coupling (d) — are now contingent. They're worth pursuing only if the project chooses to keep investing in the Lie path on the strength of a *new* hypothesis (i.e., a non-`false_acceptance` axis where Lie is expected to win). If Lie is demoted, options (a)–(d) become academic.
