...
- ⏸ Hari-assisted runs vs. baseline IX runs comparison: requires real IX-side autoresearch to actually drive the protocol against real benchmarks; not done. **Partial (2026-09-14, #13):** recorded real `ix-autoresearch` grammar runs now reach Hari through `hari-from-ix-autoresearch` (replay and `serve`-session parity pinned). On them, the default arm reproduces IX's own decision at every claim, and no config is observed twice, so no contradiction can arise. See `docs/research/hari-assisted-ix-comparison-report.md`. This is characterisation, not a verdict. A live loop following Hari's recommendation is still not built.
